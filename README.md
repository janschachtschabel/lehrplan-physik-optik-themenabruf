...
```bash
python -m unittest discover      # 36 Tests, offline, ohne Netz
```
...
```bash
python -m unittest tests.test_queries tests.test_classify tests.test_fetch tests.test_sparql_syntax
```
...
* **Live-Status.** Der erste Lauf gegen `sparql.mem.edufeed.org` schlug mit
  HTTP 500 fehl (Transitiv-Speicher, siehe Punkt 4 oben); nach dem Umbau auf
  beschränkte Pfade steht ein erneuter Live-Lauf noch aus. Die Queries sind
  syntaktisch (rdflib-Parser) und semantisch (lokaler Graph mit echten
  Ontologie-Axiomen) geprüft. Erster Lauf am besten mit `--limit 2 -v`.
